...
namespace jing 
{
    class ciFaceTracker : public BaseFaceTracker {
    public:
        ciFaceTracker();
        void setup() override;
        ...
        std::vector<ci::vec3> getImagePoints() const;
        std::vector<ci::vec3> getObjectPoints() const;
        std::vector<ci::vec3> getMeanObjectPoints() const;
    ...
    protected:
        ci::TriMesh getMesh(const std::vector<ci::vec3>& points) const;
        ci::TriMesh getMesh(const std::vector<ci::vec2>& points) const;

        void updateObjectPoints();

        static std::vector<int> getFeatureIndices(Feature feature);
        ci::PolyLine3  getFeature(Feature feature, const std::vector<ci::vec3>& points) const;
